...
class CustomPhotoCollectionViewCell: DTPhotoCollectionViewCell {
    ...
    lazy var videoButton: UIButton = {
        let videoButton = UIButton(frame: CGRect(x: 50, y: 500, width: 30, height: 30))
        videoButton.setImage(UIImage(named: "play-button"), for: .normal)
        videoButton.contentHorizontalAlignment = .center
        videoButton.contentVerticalAlignment = UIControl.ContentVerticalAlignment.center
        videoButton.contentHorizontalAlignment = UIControl.ContentHorizontalAlignment.center
//        videoButton.addTarget(self, action: #selector(videoButtonTapped(_:)), for: UIControl.Event.touchUpInside)
        return videoButton
    }()
    ...
    override func layoutSubviews() {
        ...
        videoButton.frame = CGRect(x: (bounds.size.width - 30)/2, y: (bounds.size.height - 30)/2, width: 30, height: 30)
    }
}
...
//            if let delegate = self.delegate as? SimplePhotoViewerControllerDelegate {
////                delegate.simplePhotoViewerController(self, savePhotoAt: self.currentPhotoIndex)
//                delegate.videoPlayed(index: self.currentPhotoIndex)
//            }
////        }
////        alertController.addAction(saveButton)
////        present(alertController, animated: true, completion: nil)
